...
class User extends Admin	
{
	...
	*/
	public function add_save()
	{
		...
		$code_pin =md5($this->input->post('pin'));
		$is_unique ='|is_unique[aauth_users.pin_code]';
		...
		$this->form_validation->set_rules('pin', 'pin', 'trim|required|max_length[6]|xss_clean'.$is_unique);
		...
		if ($this->form_validation->run()) {
			...
			$save_data = [
				...
				'boutique' => implode(',', (array) $this->input->post('boutique')),
			];
...
//$this->form_validation->set_rules('boutique[]', 'boutique', 'trim|required');
	...
	*/
	...
	private function _remove($id)
	{
		...
		return $this->model_user->change($id,array('delete_status_user'=>1));
	}
