...

{
...

	{
		...
		REF_PRODUCT_CODEBAR, QUANTITE, PRIX_VENDU, cp.NAME_PRODUIT as PROD_NAME
...

	{
	   ...
	   $prod_btk_Pos = $this->db->query("
			...
			SELECT DISTINCT(p.NAME_PRODUIT),p.POS_IBI_COMMANDES_ID, COUNT(*) FROM pos_ibi_commandes_produits p INNER JOIN pos_ibi_commandes c ON c.ID_POS_IBI_COMMANDES = p.POS_IBI_COMMANDES_ID WHERE p.STORE_ID_POS_IBI_COMMANDES_PRODUITS=8 AND c.CLIENT_ID_COMMANDE = ".$user_id." GROUP BY p.NAME_PRODUIT ORDER BY COUNT(*) DESC LIMIT 10")->result();
			...
			foreach ($prod_btk_Pos as $value) {
				...
				$nbre_product = $this->db->query("SELECT * FROM pos_ibi_commandes_produits p INNER JOIN pos_ibi_commandes c ON c.ID_POS_IBI_COMMANDES = p.POS_IBI_COMMANDES_ID  WHERE p.STORE_ID_POS_IBI_COMMANDES_PRODUITS=8 AND p.NAME_PRODUIT = '" . $value->NAME_PRODUIT . "' AND c.CLIENT_ID_COMMANDE = '" . $user_id . "'  ")->num_rows();



				$all_divers .= " {name: '" . $value->NAME_PRODUIT . "',y: " . number_format($nbre_product) . ",drilldown: '" . $value->NAME_PRODUIT . "'
			...
			$prod_btk_bar = $this->db->query("
				...
				SELECT DISTINCT(p.NAME_PRODUIT),p.POS_IBI_COMMANDES_ID, COUNT(*) FROM pos_ibi_commandes_produits p INNER JOIN pos_ibi_commandes c ON c.ID_POS_IBI_COMMANDES = p.POS_IBI_COMMANDES_ID WHERE p.STORE_ID_POS_IBI_COMMANDES_PRODUITS=4 AND c.CLIENT_ID_COMMANDE = ".$user_id." GROUP BY p.NAME_PRODUIT ORDER BY COUNT(*) DESC LIMIT 10")->result();
			...
			foreach ($prod_btk_bar as  $value) {
				...
				$nbre_bar = $this->db->query("SELECT * FROM pos_ibi_commandes_produits p  INNER JOIN pos_ibi_commandes c ON c.ID_POS_IBI_COMMANDES = p.POS_IBI_COMMANDES_ID WHERE p.STORE_ID_POS_IBI_COMMANDES_PRODUITS=4 AND p.NAME_PRODUIT = '" . $value->NAME_PRODUIT . "' AND c.CLIENT_ID_COMMANDE = '" . $user_id . "'  ")->num_rows();



				$all_bar .= " {name: '" . $value->NAME_PRODUIT . "',y: " . number_format($nbre_bar) . ",drilldown: '" .$value->NAME_PRODUIT. "'
			...
			$prod_btk_kichen = $this->db->query("
				...
				SELECT DISTINCT(p.NAME_PRODUIT),p.POS_IBI_COMMANDES_ID, COUNT(*) FROM pos_ibi_commandes_produits p INNER JOIN pos_ibi_commandes c ON c.ID_POS_IBI_COMMANDES = p.POS_IBI_COMMANDES_ID WHERE p.STORE_ID_POS_IBI_COMMANDES_PRODUITS=2 AND c.CLIENT_ID_COMMANDE = ".$user_id." GROUP BY p.NAME_PRODUIT ORDER BY COUNT(*) DESC LIMIT 10")->result();
			...
			foreach ($prod_btk_kichen as  $value) {
				...
				$nbre_kichen = $this->db->query("SELECT * FROM pos_ibi_commandes_produits p  INNER JOIN pos_ibi_commandes c ON c.ID_POS_IBI_COMMANDES = p.POS_IBI_COMMANDES_ID WHERE p.STORE_ID_POS_IBI_COMMANDES_PRODUITS=2 AND p.NAME_PRODUIT = '" . $value->NAME_PRODUIT . "' AND c.CLIENT_ID_COMMANDE = '" . $user_id . "'  ")->num_rows();





				$all .= " {name: '" . $value->NAME_PRODUIT. "',y: " . number_format($nbre_kichen) . ",drilldown: '".$value->NAME_PRODUIT. "'
...

	{
		...
		$prod_divers = $this->db->query("SELECT DISTINCT(p.NAME_PRODUIT),p.POS_IBI_COMMANDES_ID, COUNT(*) FROM pos_ibi_commandes_produits p  INNER JOIN client_file file ON file.CLIENT_FILE_ID = p.CLIENT_FILE_ID_COMMANDES_PRODUITS  WHERE p.REF_ID_SERVICE_COMMANDE  = 4 AND file.CLIENT_ID = " . $user_id . " GROUP BY p.NAME_PRODUIT ORDER BY COUNT(*) DESC LIMIT 10")->result();
			...
			foreach ($prod_divers as $value) {
				...
				$get_divers = $this->db->query("SELECT * FROM pos_ibi_commandes_produits p INNER JOIN client_file file ON file.CLIENT_FILE_ID = p.CLIENT_FILE_ID_COMMANDES_PRODUITS  WHERE p.NAME_PRODUIT = '" . $value->NAME_PRODUIT . "' AND file.CLIENT_ID = '" . $user_id . "' AND p.REF_ID_SERVICE_COMMANDE = 4 ")->num_rows();



				$all_divers .= " {name: '" . $value->NAME_PRODUIT . "',y: " . number_format($get_divers) . ",drilldown: '" . $value->NAME_PRODUIT . "'
