...
class Messages extends Admin
{
                ...
                if (in_array($user_id, $members)) {
                    ...
                    if ($threads[$i]->TYPE_MESSAGE_THREAD == 1 || $threads[$i]->TYPE_MESSAGE_THREAD == "1") {
                        for ($r = 0; $r < sizeof($sms); $r++) {
                            ...
                            $user = $this->db->select('*')->from('users')->where('id', $from_user)->get()->result();
                            $sms[$r]->sender = $user[0]->first_name . " " . $user[0]->last_name;
                        }
                    }
    ...
    {
        $all_users = $this->db->select('*')->from('aauth_users')->where('deleted', '0')->where('status', 'active')->get()->result();
    ...
    public function upload_file_to_temp_custom()
    {
        if (!empty($_FILES)) {
            ...
            $target_file = $target_path .  $stamp . "_" . $file_name;
            copy($temp_file, $target_file);
            return $temp_file_path .  $stamp . "_" . $file_name;
        }
    }
    ...
    public function new_send_message_file()
    {
        ...
        $target_file = $this->upload_file_to_temp_custom();
        ...
        if (sizeof($thread) > 0) {
            ...
            if (in_array(get_user_data('id'), $members)) {
                ...
                $user = $this->db->select('*')->from('users')->where('id', $from_user)->get()->result();
                $msg[0]->sender = $user[0]->first_name . " " . $user[0]->last_name;
