...
class Model_approvisionnement extends MY_Model
{
	...
	private $field_search 	=['ID_TYPE_APPROVISIONNEMENT', 'MONTAT_APPROVISIONNEMENT', 'ID_FOURNISSEUR_APPROVISIONNEMENT', 'DATE_CREATION_APPROVISIONNEMENT', 'DATE_MOD_APPROVISIONNEMENT', 'AUTHOR_APPROVISIONNEMENT'];
	...
	{
		$this->db->select('app.ID_APPROVISIONNEMENT,app.ID_TYPE_APPROVISIONNEMENT, artfl.QUANTITE_SF,artfl.UNIT_PRICE_SF,artfl.TOTAL_PRICE_SF,	artfl.REF_ARTICLE_BARCODE_SF,cl.DESIGN_ARTICLE,artfl.ID_SF, typ.DESIGN_TYPE_APPROVISIONNEMENT
		...
		$this->db->join('pos_ibi_fournisseurs f','f.ID=app.ID_FOURNISSEUR_APPROVISIONNEMENT');
	...
	{
	$this->db->distinct();
	$this->db->select('app.ID_APPROVISIONNEMENT,art.DESIGN_ARTICLE,app.DATE_CREATION_APPROVISIONNEMENT,app.DATE_MOD_APPROVISIONNEMENT,app.MONTAT_APPROVISIONNEMENT,typ.DESIGN_TYPE_APPROVISIONNEMENT,f.NOM,aut.username,artfl.UNIT_PRICE_SF,artfl.TOTAL_PRICE_SF,artfl.QUANTITE_SF,artfl.DATE_CREATION_SF,artfl.ID_SF,artfl.REF_ARTICLE_BARCODE_SF');
	$this->db->from('pos_store_'.$prefix.'_ibi_approvisionnement app');
	$this->db->join('pos_store_'.$prefix.'_ibi_articles_stock_flow artfl','app.ID_APPROVISIONNEMENT=artfl.REF_SHIPPING_SF');
	$this->db->join('pos_ibi_fournisseurs f','f.ID=app.ID_FOURNISSEUR_APPROVISIONNEMENT');
	$this->db->join('pos_store_'.$prefix.'_ibi_type_approvisionnement typ','typ.ID_TYPE_APPROVISIONNEMENT=app.ID_TYPE_APPROVISIONNEMENT');
	$this->db->join('aauth_users aut','aut.ID=app.AUTHOR_APPROVISIONNEMENT');
	$this->db->join('pos_store_'.$prefix.'_ibi_articles art','art.CODEBAR_ARTICLE=artfl.REF_ARTICLE_BARCODE_SF');
	$this->db->where('app.ID_APPROVISIONNEMENT',$id);
