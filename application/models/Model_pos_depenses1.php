...
class Model_pos_depenses extends MY_Model
{
	...
	public function count_all($q = null, $field = null)
	{
		...
		if ($this->uri->segment(2) == "index") {
			$this->db->where('ID_APPROVISIONNEMENT', NULL);
			$this->db->where('ID_REQUISITION', NULL);
			$this->db->where('ID_CATEGORIE_DEPENSE !=', 3)->or_where('ID_CATEGORIE_DEPENSE !=',2);
		} else if ($this->uri->segment(2) == "dette_envers_tiers") {
	...
	public function get($date_start, $date_end, $q = null, $field = null, $limit = 0, $offset = 0, $select_field = [])
	{
		...
		$cat_search = $this->input->get('cat_search');
		...
		if (empty($date_start) or empty($date_end)) {
			...
			if ($this->uri->segment(2) == "index") {
				...
				$this->db->where('DATE_CREATE_DEPENSE >=', $critere_start);
				$this->db->where('DATE_CREATE_DEPENSE <=', $critere_end);
			} else if ($this->uri->segment(2) == "paiement_comptant") {
