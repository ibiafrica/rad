...
class Model_fiche_travail extends MY_Model
{
	...
	public function table_name()
	{
		...
		$table_name     = 'pos_store_' . $store_prefix . '_ibi_fiche_travail pos_store_' . $store_prefix . '_ibi_fiche_travail';
		return $table_name;
	}
	...
	public function getListFilter($store, $criteres, $criteres1, $criteres2)
    {
		$where = '';
		if($criteres2 == 'null') {
			$where .= ''; 
		} else {
			$where .= 'AND TYPE_DEVIS_FICHE like "%'.$criteres2.'%"';
		}
      $query = $this->db->query('SELECT FICHE.NUMERO_FICHE,FICHE.DEVIS_CODE_FICHE,FICHE.DATE_CREATION_FICHE,
	  FICHE.REF_CLIENT_FICHE,FICHE.STATUT_FICHE,FICHE.TITRE_FICHE,FICHE.TOTAL_FICHE FROM 
	  (SELECT ft.NUMERO_FICHE,ft.DEVIS_CODE_FICHE,ft.DATE_CREATION_FICHE,ft.REF_CLIENT_FICHE,
	  ft.STATUT_FICHE,ft.TITRE_FICHE,ft.TOTAL_FICHE FROM pos_store_'.$store.'_ibi_fiche_travail ft 
	  WHERE DATE_CREATION_FICHE >="'.$criteres.'" AND DATE_CREATION_FICHE <="'.$criteres1.'" '.$where.'  AND STATUT_FICHE!=2) FICHE');
       
      if($query){
        return $query->result();
      }
    }
    public function getListFilter_count($store, $criteres, $criteres1, $criteres2)
    {
		$where = '';
		if($criteres2 == 'null') {
			$where .= ''; 
      ...
      $query = $this->db->query('SELECT FICHE.NUMERO_FICHE,FICHE.DEVIS_CODE_FICHE,FICHE.DATE_CREATION_FICHE,
	  FICHE.REF_CLIENT_FICHE,FICHE.TOTAL_FICHE FROM (SELECT ft.NUMERO_FICHE,ft.DEVIS_CODE_FICHE,
	  ft.DATE_CREATION_FICHE,ft.REF_CLIENT_FICHE,ft.TOTAL_FICHE FROM pos_store_'.$store.'_ibi_fiche_travail ft 
	  WHERE DATE_CREATION_FICHE >="'.$criteres.'" AND DATE_CREATION_FICHE <="'.$criteres1.'" '.$where.' AND STATUT_FICHE!=2) FICHE');
