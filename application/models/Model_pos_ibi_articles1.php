...
class Model_pos_ibi_articles extends MY_Model
{
    ...
    public function count_all($type_articles, $q = null, $field = null)
    {
        $iterasi = 1;
        ...
        if (empty($field)) {
            foreach ($this->field_search as $field) {
                if ($iterasi == 1) {
                    $where .= "" . $this->table_name() . "." . $field . " LIKE '%" . $q . "%' OR CONCAT(`pos_ibi_articles_categories`.`NOM_CATEGORIE`)  LIKE '%" . $q . "%'";
                } else {
    ...
    public function get($type_articles, $q = null, $field = null, $limit = 0, $offset = 0, $select_field = [])
    {
        $iterasi = 1;
        $num = count($this->field_search);
        $where = NULL;
...
if(is_null($type_articles)){
    ...
    public function get_plats($store, $q = null, $field = null, $limit = 0, $offset = 0, $select_field = [])
    {
        ...
        $query = $this->db->get('pos_store_' . $store . '_ibi_articles');

        return $query->result();
    }
    public function join_avaiable()
    {
        $this->db->join('pos_ibi_articles_categories', 'pos_ibi_articles_categories.ID_CATEGORIE = ' . $this->table_name() . '.REF_CATEGORIE_ARTICLE', 'LEFT');
...
//filtrage
  ...
  {
      # code...
          $requete = $this->db->get_where(' pos_store_1_ibi_articles',array('ID_ARTICLE'=>$product_id,'IS_INGREDIENT'=>1))->row_array();
          return $requete;
  }
     ...
     public function trouver_article($store,$id)
      {
         ...
         $this->db->FROM("pos_ibi_articles_details d,pos_store_1_ibi_articles i");
         $this->db->WHERE("i.ID_ARTICLE=d.INGREDIENT_ID  AND i.IS_INGREDIENT=1 AND  d.ARTICLE_ID=$id");
