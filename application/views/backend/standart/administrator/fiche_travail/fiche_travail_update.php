        ...
        <div class="row">
          <div class="col-md-12">
            <div class="box">
                    ...
                    <table class="table table-bordered table-striped" id="tableId">
                        <thead>
                            <tr>
                                ...
                                <td width="100">Quantité Ajouter</td>
                                <td>Sortie</td>
                                <td width="100">Cout de production</td>
                                <td>Unité</td>
                        ...
                        <tbody>
                          ...
                          $article = $this->model_registers->getOne('pos_store_'.$value['STORE_FICHE_PROD'].'_ibi_articles', array('CODEBAR_ARTICLE'=>$value['REF_PRODUCT_CODEBAR_FICHE_PROD']));
                            ...
                            $cout_production = $sortie['SORTIE'] * $value['PRIX_FICHE_PROD'];
                            $total_cout_production += $cout_production;
              ...
              <tr id="<?=$value['ID_FICHE_PROD']?>">
                <td><input type="hidden" class="codebar" name="codebar[]" value="<?=$value['REF_PRODUCT_CODEBAR_FICHE_PROD']?>"><?=$value['REF_PRODUCT_CODEBAR_FICHE_PROD']?></td>
                <td>
                  <input type="hidden" name="name[]" value="<?=$value['NAME_FICHE_PROD']?>"><?=$value['NAME_FICHE_PROD']?>
                </td>
                <td class="quantRest" hidden>
                  <input type="hidden" name="quantRest[]" value="<?=$quantiteRest?>"><?=$quantiteRest?>
                </td>
                <td class="price">
                  <input type="hidden" name="price[]" value="<?=$value['PRIX_FICHE_PROD']?>"><?=$value['PRIX_FICHE_PROD']?></td>
                <td>
                  <div class="input-group input-group-sm">
                    <span class="input-group-btn"><button type="button" class="btn btn-default moins" onclick="moins(this)" disabled><i class="fa fa-minus"></i></button>
                    </span>
                    <input readonly type="text" name="search[]" class="form-control search" onkeyup="search(this)" value="<?=$value['QUANTITE_FICHE_PROD']?>">
                    <span class="input-group-btn">
                      <button type="button" class="btn btn-default plus" onclick="plus(this)" disabled><i class="fa fa-plus"></i>
                      </button>
                    </span>
                  </div>
                </td>
                <td class="searchAdd"> 
                  <div class="input-group input-group-sm">
                    ...
                    ?>
                    <span class="input-group-btn">
                      <button <?=$disab?> type="button" class="btn btn-info btn-xs" <?=$disabled?> onclick="toAddModal(this)"><i class="fa fa-plus"></i></button>
                    </span>
                  </div>
                </td>
                <td class="quantSort"><?= number_format(round($sortie['SORTIE'], 2), 0, ' ', ' ');?></td>
                <td><?= number_format(round($cout_production, 2), 0, ' ', ' ');?></td>
          ...
          <div class="modal fade col-sm-12" id="ExModal" tabindex="+2" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
            <div class="modal-dialog" role="document"> 
                              ...
                              <div class="col-lg-6 col-md-6 col-sm-6">
                                ...
                                <div class="input-group">
                                 <div class="input-group-addon"> 
                                    ...
                                    <input type="number" class="form-control inputValue" placeholder="Quantité">
              ...
              <tfoot>
                <tr>
                  <td colspan="6">Total cout de production</td>
                  <td><?= number_format(round($total_cout_production, 2), 0, ' ', ' ');?></td>
                  <td></td>
              ...
              <td>
                <div class="form-group">
                  <div id="IMAGE_galery1"></div>
                    ...
                    <input class="data_file" name="IMAGE_name1" id="IMAGE_name1" type="hidden" value="<?= set_value('IMAGE_name1', $fiche_travail['IMAGE_PATH1']); ?>">
                </div>
              </td>
              <td>
                <div class="form-group">
                  <div id="IMAGE_galery2"></div>
                    ...
                    <input class="data_file" name="IMAGE_name2" id="IMAGE_name2" type="hidden" value="<?= set_value('IMAGE_name2', $fiche_travail['IMAGE_PATH2']); ?>">
                </div>
              </td>
              <td>
                <div class="form-group">
                  <div id="IMAGE_galery3"></div>
                    ...
                    <input class="data_file" name="IMAGE_name3" id="IMAGE_name3" type="hidden" value="<?= set_value('IMAGE_name3', $fiche_travail['IMAGE_PATH3']); ?>">
                </div>
              </td>
              <td>
                <div class="form-group">
                  <div id="IMAGE_galery4"></div>
                    ...
                    <input class="data_file" name="IMAGE_name4" id="IMAGE_name4" type="hidden" value="<?= set_value('IMAGE_name4', $fiche_travail['IMAGE_PATH4']); ?>">
                </div>
              </td>
            </tr>
          </tbody>
...
<script type="text/javascript">
  ...
  function toDelete(data){
    ...
    let sumTotal = 0;
     for(var i=0; i<table.length; i++){
       str = ($(table[i]).children()[9].firstChild.textContent);
       nbr = str.replace(" ", "");
  ...
  function search(data){
    ...
    const price = stringToNumber($(data).closest('tr').find('td.price').text());
    const quantSort = stringToNumber($(data).closest('tr').find('td.quantSort').text());
    if(initial + searchAdd < quantSort){
      alert("La quantité entrée du produit est inférieure au bon de sortie.");
      $(data).closest('tr').find('td div input.search').val(quantSort - searchAdd);
      $(data).closest('tr').find('td.total').text(formatNumber(price * (quantSort -searchAdd + searchAdd)));
    }else if(quantRest < initial + searchAdd){
        alert("La quantité restante du produit n'est pas suffisante.");
  ...
  function articleOption(){
    ...
    $('#IMAGE_galery').fineUploader({
          ...
          deleteFile: {
              ...
              endpoint: BASE_URL + '/administrator/fiche_travail/delete_IMAGE_file/<?= $this->uri->segment(4); ?>'
          },
          thumbnails: {
              placeholders: {
           ...
           session : {
             endpoint: BASE_URL + 'administrator/fiche_travail/get_IMAGE_file/<?= $this->uri->segment(4); ?>/<?= $this->uri->segment(5); ?>',
      ...
      $('#IMAGE_galery1').fineUploader({
          ...
          deleteFile: {
              enabled: true, // defaults to false
              endpoint: BASE_URL + '/administrator/fiche_travail/delete_IMAGE_file1/<?= $this->uri->segment(4); ?>'
          },
          thumbnails: {
              placeholders: {
                  waitingPath: BASE_URL + '/asset/fine-upload/placeholders/waiting-generic.png',
           ...
           session : {
             endpoint: BASE_URL + 'administrator/fiche_travail/get_IMAGE_file1/<?= $this->uri->segment(4); ?>/<?= $this->uri->segment(5); ?>',
             refreshOnRequest:true
           },
          multiple : false,
          validation: {
          ...
          callbacks: {
              ...
              onSubmit : function(id, name) {
                  ...
                  $.get(BASE_URL + '/administrator/fiche_travail/delete_IMAGE_file1/<?= $this->uri->segment(4); ?>/' + uuid1);
              },
              onDeleteComplete : function(id, xhr, isError) {
                if (isError == false) {
                  $('#IMAGE_uuid1').val('');
      ...
      $('#IMAGE_galery2').fineUploader({
          ...
          deleteFile: {
              enabled: true, // defaults to false
              endpoint: BASE_URL + '/administrator/fiche_travail/delete_IMAGE_file2/<?= $this->uri->segment(4); ?>'
          },
          thumbnails: {
              placeholders: {
                  waitingPath: BASE_URL + '/asset/fine-upload/placeholders/waiting-generic.png',
           ...
           session : {
             endpoint: BASE_URL + 'administrator/fiche_travail/get_IMAGE_file2/<?= $this->uri->segment(4); ?>/<?= $this->uri->segment(5); ?>',
             refreshOnRequest:true
           },
          multiple : false,
          validation: {
          ...
          callbacks: {
              ...
              onSubmit : function(id, name) {
                  ...
                  $.get(BASE_URL + '/administrator/fiche_travail/delete_IMAGE_file2/<?= $this->uri->segment(4); ?>/' + uuid2);
              },
              onDeleteComplete : function(id, xhr, isError) {
                if (isError == false) {
                  $('#IMAGE_uuid2').val('');
      ...
      $('#IMAGE_galery3').fineUploader({
          ...
          deleteFile: {
              enabled: true, // defaults to false
              endpoint: BASE_URL + '/administrator/fiche_travail/delete_IMAGE_file3/<?= $this->uri->segment(4); ?>'
          },
          thumbnails: {
              placeholders: {
                  waitingPath: BASE_URL + '/asset/fine-upload/placeholders/waiting-generic.png',
           ...
           session : {
             endpoint: BASE_URL + 'administrator/fiche_travail/get_IMAGE_file3/<?= $this->uri->segment(4); ?>/<?= $this->uri->segment(5); ?>',
             refreshOnRequest:true
           },
          multiple : false,
          validation: {
          ...
          callbacks: {
              ...
              onSubmit : function(id, name) {
                  ...
                  $.get(BASE_URL + '/administrator/fiche_travail/delete_IMAGE_file3/<?= $this->uri->segment(4); ?>/' + uuid3);
              },
              onDeleteComplete : function(id, xhr, isError) {
                if (isError == false) {
                  $('#IMAGE_uuid3').val('');
      ...
      $('#IMAGE_galery4').fineUploader({
          ...
          deleteFile: {
              enabled: true, // defaults to false
              endpoint: BASE_URL + '/administrator/fiche_travail/delete_IMAGE_file4/<?= $this->uri->segment(4); ?>'
          },
          thumbnails: {
              placeholders: {
                  waitingPath: BASE_URL + '/asset/fine-upload/placeholders/waiting-generic.png',
           ...
           session : {
             endpoint: BASE_URL + 'administrator/fiche_travail/get_IMAGE_file4/<?= $this->uri->segment(4); ?>/<?= $this->uri->segment(5); ?>',
             refreshOnRequest:true
           },
          multiple : false,
          validation: {
          ...
          callbacks: {
              ...
              onSubmit : function(id, name) {
                  ...
                  $.get(BASE_URL + '/administrator/fiche_travail/delete_IMAGE_file4/<?= $this->uri->segment(4); ?>/' + uuid4);
              },
              onDeleteComplete : function(id, xhr, isError) {
                if (isError == false) {
                  $('#IMAGE_uuid4').val('');
      ...
      function(isConfirm) {
        if (isConfirm) {
          $.ajax({
            ...
            success: function(data) {
              if(data == 'danger'){
                swal("Error!", "Quantite insuffisante", "error");
              }else{
