...
TITRE_DEVIS,CODE_DEVIS,COEFFICIENT_DEVIS,STATUT_DEVIS,TOTAL_FINAL_DEVIS
 ...
 FROM pos_store_2_ibi_devis where ID_DEVIS =".$devisId."");
                   /* }else{
                      show_404();
                    }*/
     ...
     $commandprods1 = $this->db->query("SELECT QUANTITE_ADD_DEVIS_PROD,PRIX_DEVIS_PROD,QUANTITE_DEVIS_PROD FROM pos_store_2_ibi_devis_produits WHERE REF_DEVIS_CODE_DEVIS_PROD='".$devisId."'");
                ...
                <div class="panel-body">
                    ...
                    <div class="table-responsive">
                        ...
                        <form method="post" id="i_form">
                            <tbody id="testarticle" hidden></tbody>
                        </form>
                        
                            <tbody id='testarticle1'>
                                <?php

                    $rek_ =$this->db->query("SELECT * FROM pos_store_2_ibi_devis_produits WHERE REF_DEVIS_CODE_DEVIS_PROD='".$devisId."' ORDER BY ID_DEVIS_PROD desc");
                                ...
                                <tr>
                                    ...
                                    <td class="text-center"><?php echo $rek['QUANTITE_DEVIS_PROD']?> </td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-danger delete" value="<?=$rek['REF_PRODUCT_CODEBAR_DEVIS_PROD']?>" prixProd="<?=$total; ?>">Supprimer</button>
      </div>
    </div>
  </div>
</div>
                    ...
                    <table cellpadding="0" cellspacing="0" style="float: right;">
                     ...
                     $total_all += $commandprod1->PRIX_DEVIS_PROD * ($commandprod1->QUANTITE_ADD_DEVIS_PROD + $commandprod1->QUANTITE_DEVIS_PROD);
                                    ...
                                    <?php        
                                            ...
                                            ?>
                      <td class="text-center"><input type="hidden" class="prixTotal" name="prixTotal" value="<?=$total_all?>"><?php echo ($total_all) ?></td>
                                </tr>
                                 <tr>
 ...
 <div class="modal-footer">
    ...
    <!-- Modal -->
                                 <tr>
                                    ...
                                    <td></td>
                                    <td></td>
                                    <td  colspan="6" class="text-center"><?php echo ($total_all * $coe) ?></td>
                                </tr>
                                 ...
                                 <tr>
                                    ...
                                    <td></td>
                                    <td></td>
                                    <td class="text-center"><a class="btn btn-default btn-sm" data-toggle="modal" data-target="#tot"><?php if($tt==0){ echo $total_all * $coe;}else{echo $tt;}
    ...
    <div class="modal-content">
     ...
     <!--  <form id="myform" method="post" accept-charset="utf-8"> -->
      <div class="modal-body" >    
          ...
          <input type="hidden" id="id" name="devis_id" value="<?php echo $devisId?>"> 
          <input type="hidden" id="store" value="<?php //echo $store ?>" id="store" name="store"> 
...
<!--        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
            <input type="submit" id="ajouter" name="ajouter" value="Enregistrer" class="btn btn-primary ajouter">
          </div>
      </form> -->
...
href="<?= site_url('administrator/pos_store_2_ibi_devis'); ?>" class="btn btn-danger" id="cancel" title="Retour"><span class="glyphicon glyphicon-remove"></span></a>
<?php if($status==0){?>
<button type="button" id="edit" class="btn btn-warning" title="Modifier"><span class="glyphicon glyphicon-edit"></span></button>
...
<?php }?>
<button style="display: none" id="editsave" type="submit" class="btn btn-primary" title="Modifier">Modifier</button>
      ...
      <div class="modal-body">
        <?php
              ...
              //$p='pos5_';
               ...
               $sqldevis=$this->db->query("SELECT CODE_DEVIS,TOTAL_FINAL_DEVIS FROM pos_store_2_ibi_devis WHERE ID_DEVIS=".$devisId."");
               foreach ($sqldevis->result() as $value) {
                 $deviscode=$value->CODE_DEVIS;
                 $totalP=$value->TOTAL_FINAL_DEVIS;
                }
               $getProduit=$this->db->query('SELECT * FROM pos_store_2_ibi_articles WHERE CODEBAR_ARTICLE not in (SELECT REF_PRODUCT_CODEBAR_DEVIS_PROD FROM pos_store_2_ibi_devis_produits WHERE REF_DEVIS_CODE_DEVIS_PROD="'.$deviscode.'")');
      

              ?>
        <input type="hidden" name="deviscode" id="deviscode" value="<?=$deviscode?>">
        <input type="hidden" name="totalP" value="<?=$totalP?>">
        <input type="hidden" name="userId" value="<?//=$userId?>"> 
        <input type="hidden" name="store_prefix" id="store" value="<?//=$store_prefix?>">
        <input type="hidden" name="storeuri" value="<?//=$storeuri?>">
        <input type="hidden" name="type_devis" id="type_devis" value="<?//=$_GET['type']?>">
        <div class="row">
    <div class="col-md-12">
...
//echo $articles->PRIX_DE_VENTE_ARTICLE;
     ...
     <li><a class="articleOption" id="<?=$articles->ID_ARTICLE ?>" quantRest="<?=$articles->QUANTITE_RESTANTE_ARTICLE ?>"  unit="<?=$articles->POIDS_ARTICLE ?>"  price="<?=$articles->PRIX_DE_VENTE_ARTICLE ?>"><?php echo $articles->DESIGN_ARTICLE; ?></a>
    ...
    <div class="col-md-12">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-primary addsave">Ajouter</button>
      </div>
    </div>
  </form>
  </div>
  ...
  function search(data){
    ...
    $(".articleOption").on("click", function(){
      ...
      $("#list").attr("hidden", 'true');
        ...
        row += '<td style="line-height: 35px;" class="article"><input type="hidden" name="article[]" value="'+articleId+'"/>'+name+'</td>';
    ...
    $('#addsave_form').on('submit', function (event) {
     ...
     $('#edit').on('click',function(){
           ...
           url: BASE_URL + '/administrator/pos_store_2_ibi_devis/select_product_devis_to_updated',
   ...
   return false;
       ...
       //avoid_multi_click_btn('btn_save', 5000);
        ...
        $.ajax({
            ...
            url: BASE_URL + '/administrator/pos_store_2_ibi_devis/cout_production_update_page_modification',
        ...
        $('.delete').on('click',function(){
           ...
           const prixProd = $(this).attr("prixProd");
           var deviscode=$('#deviscode').val();
           var type_devis=$('#type_devis').val();
           var storedelete=$('#store').val();
           var devisId=$('#devisId').val();
           var prixTotal=$('.prixTotal').val();
           $.ajax({ 
                    url: "http://gts.ibi-africa.com/ibi2/test/devis40.php",
                    method: "POST",
                    data: {codebar:codebar,deviscode:deviscode,type_devis:type_devis,storedelete:storedelete,devisId:devisId,prixProd:prixProd,prixTotal:prixTotal},
                    dataType: 'json',
                    success: function (data) {
                      if (data.message === "success") {
