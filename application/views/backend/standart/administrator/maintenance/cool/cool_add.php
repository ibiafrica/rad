                                  ...
                                  <td width="200">
                                        <select  class="form-control chosen chosen-select-deselect EQUIPMENT_NAME" id="EQUIPMENT_NAME" data-placeholder="Selectionner l'article ">
                                          ...
                                          <?php foreach (db_get_all_data('pos_store_'.$this->uri->segment(4).'_ibi_articles') as $row):
                                            ?>
                                            <option value="<?= $row->CODEBAR_ARTICLE ?>" prix="<?= $row->PRIX_DE_VENTE_ARTICLE ?>" quantRest="<?= $row->QUANTITE_RESTANTE_ARTICLE ?>" design="<?= $row->DESIGN_ARTICLE; ?>"><?= $row->DESIGN_ARTICLE; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                        <small class="info help-block">
                                        </small>
  ...
  $(document).ready(function() {
   ...
   $('#btn_save').click(function() {
      ...
      $('.message').fadeOut();
        ...
        $.ajax({
            url: BASE_URL + '/administrator/cool/add_save/<?=$this->uri->segment(4);?>',
            type: 'POST',
            dataType: 'json',
            data: data_post,
          })
