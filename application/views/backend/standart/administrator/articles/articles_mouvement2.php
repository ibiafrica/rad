            ...
            <div class="box box-warning">
                <div class="box-body ">
                    ...
                    <div class="box box-widget widget-user-2">
                      ...
                      <form name="form_mouvement" id="form_mouvement" action="<?= base_url('administrator/rapport/mouvement/'.$this->uri->segment(4).''); ?>">
                        ...
                        <div class="table-responsive"> 
                          <table class="table table-bordered table-striped" id="headerTable">
                             ...
                             $entree = $this->model_dashboard->getRequeteOne('SELECT SUM(QUANTITE_SF) AS QUANTITE_SF FROM pos_store_'.$this->uri->segment(4).'_ibi_articles_stock_flow WHERE REF_ARTICLE_BARCODE_SF="'.$article->CODEBAR_ARTICLE.'" AND TYPE_SF="stock_in" AND DATE_CREATION_SF >= "'.$date_depart.'" AND DATE_CREATION_SF <= "'.$date_fin.'"');
                             $sortie = $this->model_dashboard->getRequeteOne('SELECT SUM(QUANTITE_SF) AS QUANTITE_SF FROM pos_store_'.$this->uri->segment(4).'_ibi_articles_stock_flow WHERE REF_ARTICLE_BARCODE_SF="'.$article->CODEBAR_ARTICLE.'" AND TYPE_SF="stock_out" AND DATE_CREATION_SF >= "'.$date_depart.'" AND DATE_CREATION_SF <= "'.$date_fin.'"');  
                             
                             $reserve = $this->model_registers->get_quantite_reserve($this->uri->segment(4),$article->CODEBAR_ARTICLE,$date_search);
                             ...
                             if(!isset($reserve)){
                              ...
                              ?>
                                <tr> 
                                   ...
                                   <th><?= _ent($reserve['QTE_RESERVE']); ?></th>
                                </tr>
                              <?php endforeach; ?>
