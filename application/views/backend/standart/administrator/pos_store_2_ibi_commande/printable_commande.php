    ...
    <section id="memo">
        <div class="company-info">
          <section id="invoice-title-number">
            ...
            <br/>
            <br/>
            <span id="number"><b>N° : 4444444444444</b></span>
                    <br/>
                    <br/>
            </section>
      ...
      <section id="lemo">
         <div class="logo"><img style="" src=""></div>
                ...
                <div id="customer">
<?php
$this->db->select('commande_client_id');
            $this->db->from('pos_store_2_ibi_commande');
            $this->db->where('commande_id',$this->uri->segment(4));
            $query = $this->db->get();
            if ($query->num_rows() > 0)  //Ensure that there is at least one result 
            {
              foreach ($query->result() as $rows)  //Iterate through results
               {

$this->db->select('NOM_CLIENT,PRENOM_CLIENT,ADRESSE_CLIENT,EMAIL_CLIENT,TEL_CLIENT,STATE_CLIENT');
$this->db->from('pos_ibi_clients');
$this->db->where('ID_CLIENT',$rows->commande_client_id);
$query = $this->db->get();
if ($query->num_rows() > 0)  //Ensure that there is at least one result 
{
...
?> 
  ...
  <div class="page-footer">
            ...
            <?php $this->db->select('delai,delai_value,validite,validite_value,a_la_commande,a_la_livraison,condition_payement,commande_user_id,commande_article_decription');
            $this->db->from('pos_store_2_ibi_commande');
            $this->db->where('commande_id',$this->uri->segment(4));
            $query = $this->db->get();
            if ($query->num_rows() > 0)  //Ensure that there is at least one result 
        ...
        switch ($row->delai) 
          ...
          switch ($row->condition_payement) 
             {
              ...
              case 2:
                  $quoi=$row->a_la_commande.'% à la commande et '.$row->a_la_livraison.'% à la livraison';
              break;
              }
            ...
            switch ($row->validite) 
             {
              ...
              break;
                  ...
                  <div>
                    <span>Validité :</span> <span><?=$deadline?>.</span>
                  </div>
                  <div>
          ...
          <table style="margin:auto; width: 90%; border:0px solid;">
                    ...
                    <table cellpadding="0" cellspacing="0">
                     <thead>
                      <tr> 
                        ...
                        <th>Remise (-)</th>
                        <th>Total</th>
                        <th>Disponibilité <!-- <?php print_r(count($order[ 'products' ]));?> --> </th>
                      </tr>
                  </thead>
            ...
            $this->db->select('commande_prix,commande_quantite');
            $this->db->from('pos_store_2_ibi_commande_detail');
            $this->db->where('id_commandes',$this->uri->segment(4));
            $query = $this->db->get();
            if ($query->num_rows() > 0)  //Ensure that there is at least one result 
            {
               ...
               foreach ($query->result() as $row)  //Iterate through results
               {
                  ...
                  $total = $row->commande_prix * $row->commande_quantite;
                   $total_all += $total;
          ...
          $this->db->select('commande_article_decription');
            $this->db->from('pos_store_2_ibi_commande');
            $this->db->where('commande_id',$this->uri->segment(4));
            $query = $this->db->get();
            if ($query->num_rows() > 0)  //Ensure that there is at least one result 
          ...
          /*  $this->db->select('commande_article_decription');
            ...
            {*/
              $i=0;
               foreach ($query->result() as $row)  //Iterate through results
               {
               $article=$row->commande_article_decription;
                  $i++;
            ...
            ?> 
                    ...
                    echo convertNumberToWord($total_all).'FBU';
