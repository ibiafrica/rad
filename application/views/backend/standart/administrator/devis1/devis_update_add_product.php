...
<?php
    ...
    $store=$this->uri->segment(4);
                    ...
                    $this->db->where('REF_DEVIS_CODE_DEVIS_PROD',$devisId);
                   // $this->db->limit('1');
                    $query = $this->db->get();
                ...
                $design=$value->NAME_DEVIS_PROD;
                $qte=$value->QUANTITE_DEVIS_PROD;
                $qte_all=$value->QUANTITE_DEVIS_PROD + $value->QUANTITE_ADD_DEVIS_PROD;
                $qte_add = $value->QUANTITE_ADD_DEVIS_PROD;
                $pr = $value->PRIX_DEVIS_PROD;
                $prixt = $value->PRIX_DEVIS_PROD * $qte_all;
                

                    echo '<tr><td class="text-left '.$codebar.'"><div id="codebar">'.$codebar.'</div></td>
                    <td> <input type="hidden" value='.$codebar.' name="codebar[]">'.$design.'</td>
                    <td class="text-center"><input type="hidden" min="0" readonly name="pr[]" value='.$pr.' class="form-control pr">'.$pr.'</td><td class="text-center quant" width="100"><input type="hidden" name="Quty[]" value="'.$qte.'" class="form-control quantity">'.str_replace('.',',',$qte).'</td>
                   
                    <td class="text-center" width="100"><div><input type="number" min="0" name="qtplus[]" value='.$qte_add.' class="form-control quantityadd" onkeyup="quantityadd(this)"></div></td>
                   
                    <td class="text-center" width="100"><input type="text" min="1" readonly  name="prixt[]" value='.str_replace('.',',',$prixt).' class="form-control prixt"></td>
                    </td><td class="text-center">'.$value->UNIT_DEVIS_PROD.'</td></tr>';
                ?>
...
<!-- Modal -->
<div class="modal fade" id="exampleModal<?=$value->REF_PRODUCT_CODEBAR_DEVIS_PROD ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-body">
        Etes-vous sur de vouloir supprimer le produit <?=$value->NAME_DEVIS_PROD?> ?
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="button" class="btn btn-danger delete" value="<?=$value->REF_PRODUCT_CODEBAR_DEVIS_PROD?>">Supprimer</button>
      </div>
    </div>
  </div>
</div>
...
<button class="btn btn-flat btn-primary add_quantites_on_devis btn_action btn_save_back" id="btn_save" data-stype='back' title="Enregistrer et retourner à la liste">
