  ...
  <script>
    ...
    function playaudio() {
      ...
      if (playing !== undefined) {
        playing.then(() => {
          // var servir = prompt("Votre attention s.v.p! \n Nouvelle requisition en provenance des services.", "Je suis prêt(e) à servir")
          // if (servir != null) {
          //   audio.pause();
          // }
        ...
        success: function(data) {
          ...
          data.feedTrans.forEach((elem) => {
            if (elem.APPROUVED_ST == 1) {
              $('#dropdown_menu_request_notification').append(`<a href="<?= site_url('transfert/' . $this->uri->segment(2) . '/view'); ?>${elem.ID_ST}" class="list-group-item list-group-item-success" style="display:flex; justify-content:space-between"><span>${elem.NAME_STORE} a validé votre transfer</span><i class="pull-right fa fa-check-circle "></i>
          </a>`);
            }

            if (elem.APPROUVED_ST == 2) {
              $('#dropdown_menu_request_notification').append(`<a href="<?= site_url('transfert/' . $this->uri->segment(2) . '/view/'); ?>${elem.ID_ST}" class="list-group-item list-group-item-danger" style="display:flex; justify-content:space-between"><span>${elem.NAME_STORE} a rejetté votre transfer</span> <i class="pull-right fa fa-ban"></i>
          </a>`);
            }
