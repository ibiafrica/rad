...
if (!function_exists('db_get_all_datas')) {
	function db_get_all_datas($table_name = null, $where = false)
	{
		$ci = &get_instance();
		if ($where) {
			$ci->db->where_in($where);
		}
		$query = $ci->db->get($table_name);
...
if (!function_exists('get_name_user')) {
	function get_name_user($id)
	{
		...
		$requete = $ci->db->get('aauth_users');

		if ($requete->num_rows() > 0) {

			return $requete->row()->full_name;
		} else {
...
if (!function_exists('display_parametre_examen_module')) {
	function display_parametre_examen_module($parent, $level)
	{
		$ci = &get_instance();
		$ci->load->database();
		$ci->load->model('model_labo_parametres_examen');
		//$menu_type_id = $ci->model_menu->get_id_menu_type_by_flag($menu_type);

		$result = $ci->db->query("SELECT a.* FROM `labo_examens` a WHERE 1");

		foreach ($result->result() as $key) {

			$examen = $key->EXAMEN_ID;

			$examenee = $key->EXAMEN_NAME;


			$results = $ci->db->query("SELECT a.* FROM `labo_parametres_examen` a WHERE a.PARAMETRE_EXAMEN_ID=" . $examen);
			...
			foreach ($results->result() as $row) {
				...
				$examens = $row->PARAMETRE_NAME;

				echo "" . $examens . '<br/>';

				/*if ($row->Count > 0) {
		        	$ret .= '<li class="dd-item dd3-item" data-id="'.$row->PARAMETRE_EXAMEN_ID.'">
		                                          <div class="dd-handle dd3-handle"></div><div class="dd3-content">'._ent($key->EXAMEN_NAME);

		            if ($ci->aauth->is_allowed('')) {
				            $ret .= '<span class="pull-right"><a class="remove-data" href="javascript:void()" data-href="'.site_url('administrator/acct_compte_resultat/delete/'.$row->PARAMETRE_ID).'"><i class="fa fa-trash btn-action"></i></a>
				                </span';
		            }

		            if ($ci->aauth->is_allowed('')) {
				            $ret .= '<span class="pull-right"><a href="'.site_url('administrator/acct_compte_resultat/edit/'.$row->PARAMETRE_ID).'"><i class="fa fa-pencil btn-action"></i></a>
		                        </span>';
		            }

		            $ret .= '</div>';
					$ret .= display_parametre_examen_module($row->PARAMETRE_ID, $level + 1);
					$ret .= "</li>";
		        } elseif ($row->Count==0) {
		            $ret .= '<li class="dd-item dd3-item" data-id="'.$row->PARAMETRE_ID.'">
		                                          <div class="dd-handle dd3-handle"></div><div class="dd3-content">'._ent($row->PARAMETRE_NAME);
...
if (!function_exists('display_menu_admin')) {
	function display_menu_admin($parent, $level, $store)
	{
		$ci = &get_instance();
		...
		$menu_allow = [90, 89, 88, 28, 111, 27, 59, 35, 76];

		$autorisation = $ci->db->query("SELECT * FROM autorisation")->row()->STATUS;

		if (!is_int($store)) {

			$result = $ci->db->query("SELECT a.id, a.label,a.icon_color, a.type, a.link,a.store,a.icon, Deriv1.Count FROM `menu` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `menu` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.menu_type_id = 1 AND a.parent=" . $parent . " and active = 1 and boutique LIKE '%$store%' order by `sort` ASC")->result();
			...
			if ($result) {
				...
				foreach ($result as $row) {
					...
					$link = filter_var($row->link, FILTER_VALIDATE_URL) ? $row->link : base_url(
						str_replace('/', $store_uri, $row->link)
					);
							...
							if ($autorisation == 1) {
								if (!in_array($row->id, $menu_allow)) {
									...
									if ($ci->aauth->is_allowed($perms)) {
										...
										$ret .= display_menu_admin($row->id, $level + 1, $store);
		...
		// MENU WITHOUT STORE
			...
			$result = $ci->db->query("SELECT a.id, a.label,a.icon_color, a.type, a.link,a.store,a.icon, Deriv1.Count FROM `menu` a  LEFT OUTER JOIN (SELECT parent, COUNT(*) AS Count FROM `menu` GROUP BY parent) Deriv1 ON a.id = Deriv1.parent WHERE a.menu_type_id = 1 AND a.parent=" . $parent . " and active = 1 and store='Non' order by `sort` ASC")->result();
						...
						if ($row->Count > 0) {
							if ($autorisation == 1) {
								if (!in_array($row->id, $menu_allow)) {
									...
									if ($ci->aauth->is_allowed($perms)) {
										$ret .= '<li class="' . $active . '"> 
										        	<a href="' . site_url(str_replace('/', $store_uri, $row->link)) . '">';
										...
										$ret .= display_menu_admin($row->id, $level + 1, 1);
										$ret .= "</li>";
									}
								}
							} else {
...
if (!function_exists('theme_url')) {
	...
	{
		$ci = &get_instance();
		$active_theme = get_option('active_theme', 'ibi');
...
if (!function_exists('settings_address')) {
...
if (!function_exists('Recuperer_pourcentage')) {
	function Recuperer_pourcentage($patient_file_ids, $id_relative)
	{
		$ci = &get_instance();
		$ci->db->select('gr.DISCOUNT_PERCENT,gr.ID');
		$ci->db->from('hospital_patient_groups gr,patient_file p');
		$ci->db->where('gr.ID=p.' . $patient_file_ids);
		$ci->db->where('gr.ID=' . $id_relative);
		$ci->db->group_by('gr.ID');
		$requete = $ci->db->get();
